...
function initializeTelemetry(config) {
  ...
  telemetryInitPromise ??= (async () => {
    const debugLogger3 = createDebugLogger("OTEL");
    try {
      const { startTelemetrySdk } = await import("./sdk-impl-HLSEDAEP.js");
...
function endLLMRequestSpan(span, metadata) {
  ...
  try {
    ...
    if (metadata) {
      if (metadata.inputTokens !== void 0 && Number.isSafeInteger(metadata.inputTokens) && metadata.inputTokens >= 0) {
