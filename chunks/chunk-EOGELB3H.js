...
async function writeWorktreeSessionMarker(worktreePath, sessionId) {
  ...
  try {
    ...
    const excludePath = path2.isAbsolute(gitDir) ? path2.join(gitDir, "info", "exclude") : path2.join(worktreePath, gitDir, "info", "exclude");
    ...
    if (!existing.split(/\r?\n/).includes(rule)) {
      ...
      await fs2.writeFile(excludePath, `${existing}${sep}${rule}
`, "utf8");
    }
  } catch {
  }
...
`,
  ...
  async countWorktreeChanges(worktreePath) {
    try {
      ...
      const wtGit = simpleGit(worktreePath);
      const status = await wtGit.status();
      const tracked = status.staged.length + status.modified.length + status.deleted.length + status.renamed.length + status.created.length + status.conflicted.length;
