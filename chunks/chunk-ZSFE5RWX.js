...
var COVER_UPLOAD_KEYWORDS = /封面|海报|logo|首图|cover|poster/i;
function shouldOfferAsCoverCandidate(intentText) {
  return COVER_UPLOAD_KEYWORDS.test(intentText);
}
__name(shouldOfferAsCoverCandidate, "shouldOfferAsCoverCandidate");
...
async function runVisionBridge(params) {
  ...
  const coverUploadPromise = shouldOfferAsCoverCandidate(intent) ? uploadCoverCandidates(toConvert) : Promise.resolve([]);
...
${description}`
  ...
  const coverNote = coverUrls.length > 0 ? `[Meeting cover candidate uploaded \u2014 if the user wants one of these images as the meeting cover, use this exact URL verbatim (do not modify it): ${coverUrls.join(", ")}]
