...
__name(getAllGeminiMdFilenames, "getAllGeminiMdFilenames");
...
var MIME_LITE_MISSING_VIDEO_TYPES = /* @__PURE__ */ new Map([
  [".m4v", "video/x-m4v"]
]);
...
async function detectFileType(filePath) {
  ...
  const lookedUpMimeType = index_lite_default.getType(filePath) ?? MIME_LITE_MISSING_VIDEO_TYPES.get(ext) ?? null;
  if (lookedUpMimeType) {
...
async function processSingleFileContent(filePath, config, optionsOrOffset, legacyLimit, legacyPages) {
  ...
  try {
    ...
    const mediaMimeType = index_lite_default.getType(filePath) ?? MIME_LITE_MISSING_VIDEO_TYPES.get(path3.extname(filePath).toLowerCase()) ?? "application/octet-stream";
