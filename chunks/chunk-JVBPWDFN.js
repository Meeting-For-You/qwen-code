...
function createDaemonWorkspaceService(deps) {
  ...
  return {
    ...
    async setWorkspaceSkillEnabled(ctx, requestedSkillName, enabled) {
      ...
      const needsLegacyInactiveCheck = skill.level === "extension" && skill.status === "disabled" && skill.disabledReason === void 0;
      const disabledBySettings = needsLegacyInactiveCheck && resolveSkillSettings(loadBoundSettings(true)).disabledNames.has(
        normalizedName
      );
      if (skill.level === "extension" && skill.status === "disabled" && (skill.disabledReason === "inactive_extension" || skill.disabledReason === void 0 && !disabledBySettings)) {
        throw new WorkspaceSkillNotToggleableError(
          skill.name,
          "inactive_extension"
        );
    ...
    async setWorkspaceSkillsEnabled(ctx, requestedSkillNames, enabled) {
      ...
      for (const requestedName of requestedSkillNames) {
        ...
        if (skill.userInvocable === false) {
          ...
          const legacyInactive = skill.level === "extension" && skill.status === "disabled" && skill.disabledReason === void 0 && !disabledNames.has(normalizedName);
          if (skill.level === "extension" && skill.status === "disabled" && (skill.disabledReason === "inactive_extension" || legacyInactive)) {
            domainError = new WorkspaceSkillNotToggleableError(
              skill.name,
              "inactive_extension"
            );
      ...
      const validSkillNames = targets.flatMap(
        (target) => "skillName" in target ? [target.skillName] : []
      );
      const persisted = validSkillNames.length > 0 ? await persistDisabledSkillsBatch(
        boundWorkspace,
        validSkillNames,
        enabled,
        assertGenerationOpen
