...
`);
  ...
  async sendMessageWithReplyContext(chatId, text, replyContext) {
...
`);
  ...
  onResponseChunk(chatId, chunk, sessionId, segment) {
    ...
    if (!state) {
      ...
      if (state.buffer.length >= (this.qqConfig.bufferFlushLength ?? _QQChannel.MAX_BUFFER_LENGTH)) {
...
`);
  ...
  flushAndTrack(sessionId, buffer, state, logLabel) {
    ...
    this.sendMessageWithReplyContext(state.chatId, buffer, state.replyContext).then(() => {
...
`);
        ...
        if (current === state) {
          ...
          if (current.buffer.length >= (this.qqConfig.bufferFlushLength ?? _QQChannel.MAX_BUFFER_LENGTH)) {
            current.retryCount++;
            if (this.maxFlushRetries > 0 && current.retryCount >= this.maxFlushRetries) {
              this.streamState.delete(sessionId);
              this.flushedSessions.delete(sessionId);
...
`);
  ...
  prepareGroupMessage(event, chatId, { forceAtMention } = {}) {
    ...
    const safeContent = content.replace(/\[atMention=[^\]]*]/g, "").replace(/\[botOpenId:[^\]]*]/g, "").replace(/\[bot]/g, "");
...
`);
    ...
    const text = isSlash ? sanitizePromptText(safeCleanText) : `[atMention=${effectiveIsAtBot}]${openIdSuffix} [${safeName}${senderTag}]: ${sanitizePromptText(this.qqConfig.allowMention !== false ? safeContent : safeCleanText)}${suffixFromBotOpenId}`;
...
`);
    ...
    const text = isSlash ? sanitizePromptText(safeContent) : `[atMention=true] [${safeName}]: ${sanitizePromptText(safeContent)}`;
    this.handleInbound({
