...
var WorkspaceSkillNotToggleableError = class extends Error {
  constructor(skillName, reason, lockedScope) {
    super(
      lockedScope ? `Skill ${skillName} is locked by ${lockedScope} settings` : `Skill ${skillName} is not toggleable: ${reason}`
    );
    this.skillName = skillName;
    this.reason = reason;
  ...
  static {
    __name(this, "WorkspaceSkillNotToggleableError");
  }
};
function mapWorkspaceSkillToggleError(error) {
  ...
  if (error instanceof WorkspaceSkillNotToggleableError) {
    return {
      skillName: error.skillName,
      code: error.reason === "inactive_extension" ? "skill_inactive_extension" : "skill_not_toggleable",
      error: error.message,
      reason: error.reason,
