...
function formatGoalState(snapshot, operation) {
  ...
  return (goal.status === "blocked" || goal.status === "usage_limited") && goal.lastReason ? `${summary}
Reason: ${goal.lastReason}` : summary;
}
...
async function runNonInteractive(config, settings, input, prompt_id, options = {}) {
  return promptIdContext.run(prompt_id, async () => {
    ...
    const goalHost = {
      startGoalTurn: /* @__PURE__ */ __name(async (input2) => {
        if (queuedGoalTurns.some(
          ({ permit }) => permit.turnId === input2.permit.turnId
        )) {
          return;
        }
        queuedGoalTurns.push({
          permit: { ...input2.permit },
          turnKey: `goal-runtime:${input2.permit.turnId}`,
          controller: new AbortController(),
          origin: "runtime",
          continuationContext: input2.continuationContext,
    ...
    const failClosedActiveGoalTurn = /* @__PURE__ */ __name((reason) => {
...
${reason}
            ...
            event.respond("cancel" /* Cancel */).catch((err) => {
...
`);
      ...
      const processToolCallBatch = /* @__PURE__ */ __name(async (batchRequests, setModelOverride, runtimeView) => {
        ...
        const finalizeToolCall = /* @__PURE__ */ __name((requestInfo, toolResponse) => {
          ...
          config.getGeminiClient().recordCompletedToolCall(
            requestInfo.name,
            requestInfo.args
          );
      ...
      while (true) {
        ...
        for await (const event of responseStream) {
          ...
          if (outputFormat === "text" /* TEXT */ && event.type === "error" /* Error */) {
...
`);
          ...
          const memoryTaskPromises = config.getGeminiClient().consumePendingMemoryTaskPromises();
