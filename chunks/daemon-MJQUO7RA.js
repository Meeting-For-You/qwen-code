...
var DaemonSessionClient = class _DaemonSessionClient {
  ...
  async reattach() {
    ...
    this.reattaching = this.client.resumeSession(this.sessionId, { workspaceCwd: this.workspaceCwd }).then((session) => {
  ...
  switch (event.type) {
    ...
    case "session_snapshot":
      return {
        ...
        ...event.data.currentApprovalMode != null ? { approvalMode: event.data.currentApprovalMode } : {},
...
function isSessionMetadataUpdatedData(value) {
  if (!isRecord(value) || !isNonEmptyString(value["sessionId"]) || !isOptionalStringOrNull(value["displayName"])) {
    return false;
  }
  const prs = value["prs"];
...
function isDaemonSkillToggleMutation(value) {
  ...
  ) && (activation === "applied" || activation === "deferred" || activation === "partial") && isFiniteNumber(value["sessionsRefreshed"]) && isFiniteNumber(value["sessionsFailed"]);
}
...
function normalizeSessionUpdate(event, base, opts) {
  ...
  switch (kind) {
    ...
    case "tool_call_update": {
      ...
      if (getString2(update, "status") === "in_progress" && getString2(update, "kind") === void 0 && meta?.["shellProgress"] !== void 0) {
        return [];
      }
...
function userBlockForAttachment(next, event) {
  ...
  if (activeUser?.kind === "user" && stringArraysEqual(activeUser.sourceRecordIds, event.sourceRecordIds)) {
    const block2 = getWritableBlockById(next, activeUser.id);
    if (block2?.kind === "user") return block2;
  }
  const block = createTextBlock(
    next,
    "user",
...
function isLegacySubagentUsageDuplicate(state, event) {
...
__name(isLegacySubagentUsageDuplicate, "isLegacySubagentUsageDuplicate");
...
function appendUserShellBlock(state, event) {
  if (!event.text) return;
  const last = state.blocks[state.blocks.length - 1];
  if (last?.kind === "user_shell" && last.stream === event.stream && !state.pendingUserShellCommand) {
    const writable = getWritableBlockById(state, last.id);
...
function createTextBlock(state, kind, text, eventId, serverTimestamp, meta, sourceRecordIds, promptId) {
  const blockId = allocateBlockId(state, kind);
  return {
    id: blockId,
    kind,
