...
`);
  ...
  async processDirectLane(ctx, reason) {
    ...
    const envelope = {
      ...
      displayText: reason === "review_requested" ? `Review requested: ${displayTitle}` : `Issue assigned: ${displayTitle}`,
      isGroup: true,
      isMentioned: true,
      isReplyToBot: false,
...
${details}`
  ...
  async processAggregateLane(ctx) {
    ...
    const summary = comments.map((comment) => `- @${comment.user?.login || "unknown"}: ${sanitizeDisplayText((comment.body || "").trim(), MAX_AGGREGATE_COMMENT_CHARS)}`).join("\n");
    const envelope = {
      channelName: this.name,
