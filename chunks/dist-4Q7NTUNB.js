...
var DwsClient = class {
  ...
  async subscribeToIm(source, onMessage, onError) {
    ...
    return this.eventStarter(this.executable, args, async (line) => onMessage(parseDwsImEvent(line)), onError);
  }
...
function isPendingMessage(value) {
  ...
  const pending = value;
  const sourceValid = pending.source?.kind === "direct" || pending.source?.kind === "group-all" || pending.source?.kind === "group" && typeof pending.source.conversationId === "string" && Boolean(pending.source.conversationId);
  const message = pending.message;
  if (!sourceValid || typeof message !== "object" || message === null) {
    return false;
  }
  return (message.type === "user_im_message_receive_o2o" || message.type === "user_im_message_receive_o2o_all" || message.type === "user_im_message_receive_group" || message.type === "user_im_message_receive_group_all") && [
    message.type,
    message.eventId,
    message.messageId,
...
var DwsChannel = class extends PollingChannelBase {
  ...
  constructor(name, config, bridge, options, client) {
    ...
    const imSources = [{ kind: "at" }, ...groupSources];
  ...
  validateCursor(parsed) {
    ...
    return {
      ...
      pendingMessages: (cursor.pendingMessages ?? []).slice(-MAX_PROCESSED_ITEMS),
  ...
  async connect() {
    ...
    this.connected = true;
    try {
      await Promise.all(this.imStates.map((state) => this.startImSourceWithRetry(state, generation)));
    ...
    for (const reactions of this.sessionReactionKeys.values()) {
      for (const [key, { conversationId, messageId }] of reactions) {
        if (!this.activeReactionKeys.delete(key))
          continue;
        void this.client.removeImReaction(conversationId, messageId, ACK_REACTION_NAME).catch((error) => this.logReactionFailure("disconnect reaction removal", error));
      }
    }
...
`);
    ...
    await this.client.replyToImMessage(chatId, messageId, senderId, text, stableUuid(`${this.name}\0${chatId}\0${messageId}\0${text}`));
  }
  async pollOnce() {
...
`);
    state.retryTimer = setTimeout(() => {
      ...
      if (!this.connected)
        return;
      state.lastError = void 0;
      void this.startImSource(state, this.lifecycleGeneration).catch((error2) => {
...
`);
  ...
  async handleImMessage(source, message, fromHistory = false) {
    if (!this.connected)
      return;
    if (this.isSelfMessage(message)) {
      this.markProcessedMessage(messageKey(message));
    ...
    if (!fromHistory && message.eventTime !== void 0 && message.eventTime < this.connectionStartedAt - 5e3) {
    ...
    if (source.kind === "group-all" && (this.config.groups[message.conversationId]?.requireMention ?? this.config.groups["*"]?.requireMention ?? true)) {
      return;
    }
    if ((source.kind === "group" || source.kind === "group-all") && this.config.groupPolicy === "pairing" && !this.groupGate.isGroupApproved(message.conversationId)) {
      return;
    }
    const key = messageKey(message);
    while (true) {
...
`);
  ...
  rememberPendingMessage(source, message) {
    const key = messageKey(message);
    const pending = this.cursor.pendingMessages ?? [];
    if (pending.some((item) => messageKey(item.message) === key))
      return;
    while (pending.length >= MAX_PROCESSED_ITEMS) {
      ...
      process2.stderr.write(`[Channel:${this.name}] dropping the oldest pending DWS message because the retry queue reached ${MAX_PROCESSED_ITEMS}.
`);
    }
  ...
  removePendingMessage(key) {
    this.cursor.pendingMessages = (this.cursor.pendingMessages ?? []).filter((pending) => messageKey(pending.message) !== key);
  }
  async processDocumentNotification(message, key, notification) {
    ...
    if (inFlight) {
      await inFlight;
      if (this.cursor.processedMessages.includes(notificationKey) || this.hasPendingDocumentNotification(notificationKey) && !this.gate.isAllowed(message.senderId)) {
        this.markProcessedMessage(key);
      } else {
...
`);
  ...
  hasPendingDocumentNotification(notificationKey) {
    return (this.cursor.pendingDocumentNotifications ?? []).some((pending) => documentNotificationKey(pending) === notificationKey);
  }
...
`);
  ...
  startReaction(conversationId, messageId, sessionId) {
    ...
    void this.client.addImReaction(target.conversationId, target.messageId, ACK_REACTION_NAME).then(() => {
      if (!this.activeReactionKeys.has(key)) {
        void this.client.removeImReaction(target.conversationId, target.messageId, ACK_REACTION_NAME).catch((error) => this.logReactionFailure("late reaction removal", error));
      }
    }).catch((error) => {
  ...
  stopReaction(conversationId, messageId, sessionId) {
    if (!messageId)
      return;
    const target = this.inboundReactionTargets.get(this.reactionKey(conversationId, messageId));
    if (!target)
    ...
    if (!this.activeReactionKeys.delete(key))
      return;
    void this.client.removeImReaction(target.conversationId, target.messageId, ACK_REACTION_NAME).catch((error) => this.logReactionFailure("reaction removal", error));
  }
  ...
  onSessionDied(sessionId) {
    ...
    if (reactions) {
      ...
      for (const [key, { messageId, conversationId }] of reactions) {
        if (!this.activeReactionKeys.delete(key))
          continue;
        void this.client.removeImReaction(conversationId, messageId, ACK_REACTION_NAME).catch((error) => this.logReactionFailure("session-death reaction removal", error));
      }
    }
