...
function extractGeminiRequestAttributes(request) {
  const record = request;
...
__name(extractGeminiRequestAttributes, "extractGeminiRequestAttributes");
...
function reportGeminiRequest(request, requestContext) {
  return reportRequest(
    request,
...
__name(reportGeminiRequest, "reportGeminiRequest");
...
function reportGeminiResponse(handle, response) {
  handle?.controller.record(
    handle,
    (output) => output.recordGeminiResponse(response)
  );
}
__name(reportGeminiResponse, "reportGeminiResponse");
function reportGeminiChunk(handle, chunk) {
  handle?.controller.record(
    handle,
    (output) => output.recordGeminiChunk(chunk)
  );
}
__name(reportGeminiChunk, "reportGeminiChunk");
