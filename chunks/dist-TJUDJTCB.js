...
var DingtalkInteractiveCardClient = class {
  ...
  async createAndDeliver(input) {
    ...
    if (Array.isArray(deliveries)) {
      ...
      if (failure) {
        throw new Error(`${input.templateId}: ${typeof failure.errorMsg === "string" && failure.errorMsg.trim() ? failure.errorMsg.trim() : "DingTalk card delivery failed"}`);
      }
    }
  }
...
function parseDingtalkInteractiveCardConfig(value) {
  ...
  const timeoutMs = question?.["timeoutMs"] === void 0 ? DEFAULT_QUESTION_TIMEOUT_MS : question["timeoutMs"];
  if (typeof timeoutMs !== "number" || !Number.isFinite(timeoutMs) || timeoutMs <= DINGTALK_INTERACTIVE_CARD_TIMEOUT_EXCLUSIVE_MINIMUM) {
...
var StatusCardController = class {
  ...
  async flushPending(segmentId) {
    ...
    while (!record.terminal && !record.streamFailed) {
      ...
      if (record.pendingSnapshot === void 0)
        break;
    }
  ...
  flush(record) {
    if (record.terminal || record.inFlight || record.pendingSnapshot === void 0)
      return;
  ...
  statusLine(record, state) {
    ...
    return {
      text: [state, model, `${second}s`].filter(Boolean).join(" \xB7 "),
      second
    };
  }
    ...
    try {
      await this.options.client.updateInstance({
        ...
        cardParamMap: {
          ...
          ...record.terminalDescription ? { question_desc: record.terminalDescription } : {}
        }
      });
    } catch (error) {
...
var DingtalkInteractionPresenter = class {
  ...
  closeOutput(segmentId, text, reason, segment) {
    ...
    return this.enqueue(run, async () => {
      ...
      if (reason === "response_boundary") {
        ...
        const fallbackText2 = stripPartialImageMarker(text || presentation.content);
        if (!fallbackText2 || !this.options.sendFallback)
          return false;
        await this.options.sendFallback(presentation.context.target.chatId, fallbackText2, presentation.context.sessionId);
        return true;
      }
      if (reason === "input_requested") {
        const completed2 = statusCards !== void 0 && await statusCards.complete(statusContext.segmentId, this.withSenderPrefix(run, text || presentation.content));
      ...
      statusCards?.ensure(statusContext, this.cardTarget(statusContext.target));
      const completed = statusCards !== void 0 && await statusCards.complete(statusContext.segmentId, this.withSenderPrefix(run, text || presentation.content));
      if (completed)
        return true;
  ...
  ensureStatusContext(run, segment) {
    ...
    run.statusContext = segment ? { ...segment } : { ...run.baseContext, segmentId: `${run.runId}:${randomUUID4()}` };
    return run.statusContext;
  }
  cardTarget(target) {
...
var DingtalkChannel = class _DingtalkChannel extends ChannelBase {
  ...
  constructor(name, config, bridge, options) {
    ...
    if (this.interactiveCardConfig.enabled) {
      ...
      if (this.interactiveCardConfig.statusCard.enabled && config.blockStreaming !== "on") {
...
`);
      ...
      if (this.interactiveCardConfig.questionCard.enabled) {
        this.questionCardController = new QuestionCardController({
          ...
          sendFallback: /* @__PURE__ */ __name((chatId, text) => this.sendMessage(chatId, text), "sendFallback"),
      ...
      if (this.statusCardController || this.questionCardController) {
        this.interactionPresenter = new DingtalkInteractionPresenter({
          ...
          ...config.blockStreaming !== "on" ? {
            sendFallback: /* @__PURE__ */ __name((chatId, text, sessionId) => this.sendFallbackReply(chatId, text, sessionId), "sendFallback")
          } : {}
        });
      }
    }
...
`);
  ...
  async prepareOutgoingText(text) {
    ...
    for (const marker of markers) {
      const fileName = basename2(marker.path).replace(/[\r\n[\]]+/g, "_").slice(0, 100) || "image";
      try {
...
` : "";
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      const isMention = i === 0 && atUserId !== void 0;
      const body = {
        msgtype: "markdown",
        markdown: {
...
`);
  ...
  startReaction(chatId, messageId, sessionId) {
    ...
    this.attachReaction(messageId, chatId).then(() => {
      if (!this.activeReactionKeys.has(key)) {
  ...
  extractQuotedContext(data) {
    ...
    if (data.quoteMessage) {
      ...
      return { referencedText: text || void 0, isReplyToBot };
    }
...
`);
    ...
    if (!media)
      return;
    const inlineImageSlotFree = !(envelope.attachments || []).some((attachment) => attachment.type === "image" && attachment.data);
    if (mediaType === "image" && inlineImageSlotFree) {
      ...
      let safeName;
      try {
        dir = join(tmpdir2(), "channel-files", randomUUID5());
        mkdirSync(dir, { recursive: true });
        safeName = basename2(typeof fileName === "string" ? fileName : "") || `dingtalk_${mediaType}_${Date.now()}.${GENERATED_MEDIA_EXT[media.mimeType] ?? "bin"}`;
...
`);
      ...
      const processMessage = /* @__PURE__ */ __name(async () => {
        if (content.downloadCodes.length > 0 && content.mediaType) {
          await this.attachMedia(envelope, content.downloadCodes[0], content.mediaType, content.fileName, content.placeholder);
        }
        if (quoted.media) {
          await this.attachMedia(envelope, quoted.media.downloadCode, quoted.media.mediaType, quoted.media.fileName);
        }
