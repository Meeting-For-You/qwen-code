...
var ChannelBase = class {
  ...
  bridgeBackgroundResponseListener = /* @__PURE__ */ __name((sessionId, text) => {
...
`);
  ...
  async dispatchBackgroundResponse(sessionId, text) {
    ...
    if (!target || target.channelName !== this.name || text.trim().length === 0) {
      return;
    }
...
`);
  ...
  requestActivePromptCancellation(sessionId, reason = "cancel_command") {
    ...
    return cancelRequested.finally(() => {
      ...
      if (!cancelSucceeded || active.deliveryStarted || turnEnded && !active.cancelled) {
        return false;
      }
    ...
    if (active) {
      switch (mode) {
        case "collect": {
          ...
          const bufferedDisplayText = (envelope.isGroup || this.config.sessionScope === "single") && !envelope.alreadyPrefixed && !recognizedSlashCommand ? `[${sanitizeSenderName(envelope.senderName || envelope.senderId || "unknown")}] ${sanitizePromptText(displayText)}` : displayText;
          buffer.push({
            text: promptText,
...
`);
      ...
      const streamer = useBlockStreaming ? new BlockStreamer({
        ...
        send: /* @__PURE__ */ __name((text) => this.sendResponseMessage(envelope.chatId, text, sessionId), "send")
      }) : null;
  ...
  handleSessionUpdate(sessionId, data) {
    ...
    switch (type) {
      ...
      case "tool_call_update": {
        ...
        const meta = isRecord3(update["_meta"]) ? update["_meta"] : void 0;
        if (!kind && toolCallId && getString(update["status"]) === "in_progress" && meta?.["shellProgress"] !== void 0) {
          break;
        }
        if (!toolCallId || !kind) {
