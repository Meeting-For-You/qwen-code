...
var VISION_BRIDGE_MAX_IMAGES_PER_TURN = 20;
...
function buildIntentPart(intentText, sourceContext, imageParts = [], imageNumberOffset = 0) {
  const sourceHint = sourceContext ? `The images are consecutive pages ${sourceContext.renderedRange.firstPage}-${sourceContext.renderedRange.lastPage} from PDF ${JSON.stringify(sourceContext.displayName)}. Transcribe each page separately and label each section with its original PDF page number.` : imageParts.length > 0 ? `Describe each image separately under these ordered labels: ${imageParts.map(
    (part, index) => `${imageNumberOffset + index + 1}. ${JSON.stringify(part.inlineData?.displayName?.trim() || `image ${imageNumberOffset + index + 1}`)}`
  ).join("; ")}.` : "";
...
function sliceSourceContext(sourceContext, batchStart, batchLength, totalImages) {
  if (!sourceContext) return void 0;
  ...
  return {
    ...
    ...isLastBatch && sourceContext.continuation && { continuation: sourceContext.continuation }
  };
}
__name(sliceSourceContext, "sliceSourceContext");
...
async function uploadCoverCandidates(imageParts) {
  const uploadUrl = process.env["AGENT_COVER_UPLOAD_URL"];
  if (!uploadUrl) return [];
  const results = await Promise.allSettled(
    imageParts.map(async (part) => {
      const inlineData = part.inlineData;
      if (!inlineData?.data || !inlineData?.mimeType) return void 0;
      ...
      return typeof body?.url === "string" ? body.url : void 0;
    })
  );
...
__name(uploadCoverCandidates, "uploadCoverCandidates");
function buildCoverCandidateNote(coverUrls) {
  return `[Meeting cover candidate(s) uploaded \u2014 these are NOT confirmed as the meeting cover yet. Judge from the image content whether one looks like a suitable meeting cover (e.g. an official notice/poster with a title, date, venue, or organizer), and if so, proactively ask the user whether to use it \u2014 do not wait for the user to bring it up, and do not assume silence means yes. Only after the user confirms, use the exact URL verbatim (do not modify it): ${coverUrls.join(", ")}]`;
}
__name(buildCoverCandidateNote, "buildCoverCandidateNote");
async function maybeAnnotateCoverCandidates(params) {
...
__name(maybeAnnotateCoverCandidates, "maybeAnnotateCoverCandidates");
async function runVisionBridge(params) {
  ...
  const descriptions = [];
  for (let batchStart = 0; batchStart < toConvert.length; batchStart += VISION_BRIDGE_MAX_IMAGES) {
