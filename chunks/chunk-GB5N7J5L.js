...
var DeepSeekOpenAICompatibleProvider = class extends DefaultOpenAICompatibleProvider {
  ...
  buildRequest(request, userPromptId) {
    ...
    const reshaped = isDeepSeekHostname(this.contentGeneratorConfig) ? translateReasoningEffort(baseRequest) : baseRequest;
...
var StreamInactivityTimeoutError = class extends Error {
  constructor(idleMs, chunksReceived, streamLifetimeMs) {
    super(
      `No stream activity for ${idleMs}ms after ${chunksReceived} chunks (stream lifetime: ${streamLifetimeMs}ms). Set ${QWEN_STREAM_IDLE_TIMEOUT_MS_ENV} to increase this window (or 0 to disable it).`
    );
    this.idleMs = idleMs;
...
var ContentGenerationPipeline = class {
  ...
  async buildRequest(request, userPromptId, context, isStreaming) {
    ...
    if (isDashScope && typed["tool_choice"] === "required" && (thinkingMandatory || isQwenFamilyWireModel(model) && (typed["enable_thinking"] === true || thinkingBudget != null && typed["enable_thinking"] !== false || typeof reasoningEffort === "string" && reasoningEffort !== "none"))) {
      debugLogger3.debug(
  ...
  buildGenerateContentConfig(request) {
    ...
    if (configSamplingParams !== void 0) {
      ...
      return clampProviderOutputBudgetKeys(
        maxTokens !== void 0 ? { ...configSamplingParams, max_tokens: maxTokens } : { ...configSamplingParams },
        requestMaxTokens
      );
    }
    const params = {
  ...
  async executeWithErrorHandling(request, userPromptId, isStreaming, executor) {
    ...
    let openaiRequest;
    const executeAttempt = /* @__PURE__ */ __name(async () => {
...
var OpenAIContentGenerator = class {
  ...
  async embedContent(request) {
    ...
    if (Array.isArray(request.contents)) {
      text = request.contents.map((content) => {
      ...
      if (typeof request.contents === "string") {
        ...
        text = request.contents.parts.map(
          (part) => typeof part === "string" ? part : "text" in part ? part.text : ""
        ).join(" ");
      }
    }
