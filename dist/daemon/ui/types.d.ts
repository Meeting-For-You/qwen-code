 ...
 */
import type { DaemonAuthDeviceFlowSdkErrorKind, DaemonAuthProviderId, DaemonEvent, DaemonErrorKind, DaemonSessionArtifactChange, DaemonSkillToggleMutation, PermissionResponse } from '../types.js';
export declare const DAEMON_PLAN_TOOL_CALL_ID = "daemon-plan";
export type DaemonUiEventType = 'user.text.delta' | 'user.image.delta' | 'user.file.delta' | 'user.shell.command' | 'assistant.text.delta' | 'assistant.done' | 'assistant.usage' | 'thought.text.delta' | 'tool.update' | 'shell.output' | 'user.shell.output' | 'permission.request' | 'permission.resolved' | 'model.changed' | 'status' | 'error' | 'debug' | 'session.metadata.changed' | 'session.artifact.changed' | 'session.approval_mode.changed' | 'session.available_commands' | 'session.state_resync_required' | 'session.replay_complete' | 'session.rewound' | 'session.branched' | 'prompt.cancelled' | 'followup.suggestion' | 'workspace.memory.changed' | 'workspace.agent.changed' | 'workspace.tool.toggled' | 'workspace.settings.changed' | 'workspace.trust.change.requested' | 'workspace.initialized' | 'workspace.github.setup.completed' | 'workspace.mcp.budget_warning' | 'workspace.mcp.child_refused' | 'workspace.mcp.server_restarted' | 'workspace.mcp.server_restart_refused' | 'workspace.mcp.server_changed' | 'workspace.extensions.changed' | 'auth.device_flow.started' | 'auth.device_flow.throttled' | 'auth.device_flow.authorized' | 'auth.device_flow.failed' | 'auth.device_flow.cancelled';
 ...
 */
export declare const DAEMON_UI_UNRECOGNIZED_DIAGNOSTIC_REASONS: readonly ["unrecognized_event", "unrecognized_session_update"];
export type DaemonUnrecognizedDiagnosticReason = (typeof DAEMON_UI_UNRECOGNIZED_DIAGNOSTIC_REASONS)[number];
/**
 ...
 */
export declare function isUnrecognizedDiagnosticReason(reason: DaemonUiDebugReason | string | undefined): reason is DaemonUnrecognizedDiagnosticReason;
/**
...
export interface DaemonTranscriptStore {
    ...
    appendLocalUserMessage(text: string, images?: Array<{
        data: string;
        mimeType: string;
    }>, meta?: DaemonTextDeltaMeta, files?: Array<{
        name: string;
        mimeType: string;
        data?: Blob;
        text?: string;
