...
const daemon = require("@qwen-code/sdk/daemon");
...
function extractHttpStatus(error) {
  if (error instanceof daemon.DaemonHttpError) return error.status;
...
function isInvalidClientIdError(error) {
  return error instanceof daemon.DaemonHttpError && error.status === 400 && isRecord(error.body) && error.body["code"] === "invalid_client_id";
}
...
function isDaemonSessionDisconnectedError(error) {
  return error instanceof daemon.DaemonTransportClosedError || error instanceof TypeError && /(?:fetch failed|failed to fetch|networkerror|load failed)/i.test(
    error.message
  );
}
...
function createDaemonSessionActions({
  ...
  function isDefinitePromptAdmissionRejection(error) {
    return error instanceof daemon.DaemonHttpError || error instanceof daemon.DaemonPendingPromptLimitError;
  }
  ...
  return {
    async sendPrompt(text, options) {
      ...
      try {
        ...
        if (daemon.isDaemonTurnError(error)) {
          throw error;
        }
    ...
    async branchSession(name, atRecordId) {
      ...
      try {
        ...
        if (daemon.isStaleBranchPointError(error)) {
...
function isRecoverableLiveJournalMarker(event) {
  const knownEvent = daemon.asKnownDaemonEvent(event);
  ...
  return {
    ...
    async installExtensionArchive(params, clientId) {
      ...
      return withActionTimeout(
        ...
        "Install extension timed out",
        daemon.EXTENSION_ARCHIVE_UPLOAD_TIMEOUT_MS + 1e4
      );
    },
...
const DaemonWorkspaceContext = react.createContext(void 0);
...
function DaemonWorkspaceProvider({
  ...
  const client = react.useMemo(
    () => autoConnect ? new daemon.DaemonClient({ baseUrl, token, transport }) : void 0,
    [autoConnect, baseUrl, token, transport]
  );
  const clientRef = react.useRef(client);
  clientRef.current = client;
  const capabilitiesClientRef = react.useRef(void 0);
  const capabilitiesPromiseRef = react.useRef(void 0);
  const capabilitiesGenerationRef = react.useRef(0);
  const resolvedCwdRef = react.useRef(workspaceCwd);
  const [capabilities, setCapabilities] = react.useState(void 0);
  const [status, setStatus] = react.useState(
    autoConnect ? "connecting" : "idle"
  );
  const [error, setError] = react.useState(void 0);
  const getCapabilities = react.useCallback(() => {
    if (!client) {
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(DaemonWorkspaceContext.Provider, { value: contextValue, children });
}
function useDaemonWorkspace() {
  const context = react.useContext(DaemonWorkspaceContext);
  if (!context) {
    throw new Error(
...
function useOptionalDaemonWorkspace() {
  return react.useContext(DaemonWorkspaceContext);
}
/**
 * @license
 * Copyright 2025 Qwen Team
...
function parseSidechannelMidTurnInjected(event) {
  if (!event || typeof event !== "object") return void 0;
  const record = event;
  if (record["type"] !== daemon.MID_TURN_MESSAGE_INJECTED_EVENT) return void 0;
  const data = record["data"];
  if (!data || typeof data !== "object") return void 0;
  const dataRecord = data;
  const sessionId = dataRecord["sessionId"];
...
function publishPendingPromptEvent(event) {
  ...
  notifyPendingPromptEvents();
  if (parsed.type === daemon.PENDING_PROMPT_ADDED_EVENT || parsed.type === daemon.PENDING_PROMPT_STARTED_EVENT || parsed.type === daemon.PENDING_PROMPT_COMPLETED_EVENT) {
...
function parsePendingPromptEvent(event) {
  ...
  if ((type === daemon.PENDING_PROMPT_ADDED_EVENT || type === daemon.PENDING_PROMPT_STARTED_EVENT || type === daemon.PENDING_PROMPT_COMPLETED_EVENT) && typeof dataRecord["promptId"] === "string") {
    return event;
  }
...
function assistantDoneFromTurnEvent(event, reason) {
  const serverTimestamp = daemon.extractServerTimestamp(event);
...
function materializeTranscriptHistory(current, events2, maxBlocks) {
  ...
  const historyStore = daemon.createDaemonTranscriptStore({
  ...
  for (const block of pageBlockList) {
    pageBytes += daemon.estimateDaemonTranscriptBlockBytes(block);
  }
...
function applyTranscriptHistory(current, history) {
  ...
  for (const [callId, blockId] of Object.entries(current.toolBlockByCallId)) {
    if (daemon.isTrimmedToolBlockId(blockId) && toolBlockByCallId[callId] !== void 0) {
      continue;
    }
  ...
  for (const [requestId, blockId] of Object.entries(
    ...
    if (daemon.isTrimmedPermissionBlockId(blockId) && permissionBlockByRequestId[requestId] !== void 0) {
      continue;
    }
  ...
  return {
    ...
    ].slice(-daemon.UNRECOGNIZED_DIAGNOSTICS_LIMIT)
  };
}
...
const DaemonStoreContext = react.createContext(
  void 0
);
const DaemonConnectionContext = react.createContext(void 0);
const DaemonActionsContext = react.createContext(
  void 0
);
const DaemonTranscriptHistoryContext = react.createContext(void 0);
const DaemonPromptStatusContext = react.createContext(
  void 0
);
const DaemonSessionNoticesContext = react.createContext(void 0);
const DaemonWorkspaceEventSignalsContext = react.createContext(void 0);
const DaemonSessionOwnerGuardContext = react.createContext(void 0);
...
function DaemonSessionProvider(props) {
  ...
  const sessionCapabilitiesRef = react.useRef(
    workspace == null ? void 0 : workspace.capabilities
  );
  const workspaceClientRef = react.useRef(workspace == null ? void 0 : workspace.client);
  workspaceClientRef.current = workspace == null ? void 0 : workspace.client;
  const workspaceCapabilitiesRef = react.useRef(workspace == null ? void 0 : workspace.capabilities);
  workspaceCapabilitiesRef.current = workspace == null ? void 0 : workspace.capabilities;
  const workspaceGetCapabilitiesRef = react.useRef(workspace == null ? void 0 : workspace.getCapabilities);
  workspaceGetCapabilitiesRef.current = workspace == null ? void 0 : workspace.getCapabilities;
  const workspaceAcpPreheatInFlightRef = react.useRef(false);
  const initialRestoreSessionIdRef = react.useRef(sessionId);
  ...
  const resolvedWorkspaceCwdRef = react.useRef(resolvedWorkspaceCwd);
  resolvedWorkspaceCwdRef.current = resolvedWorkspaceCwd;
  const activeWorkspaceCwdRef = react.useRef(resolvedWorkspaceCwd);
  if (resolvedWorkspaceCwd) {
    activeWorkspaceCwdRef.current = resolvedWorkspaceCwd;
  }
  const sessionRef = react.useRef(void 0);
  const sessionConfigGenerationRef = react.useRef(
    /* @__PURE__ */ new WeakMap()
  );
  const transcriptHistoryRef = react.useRef({
    hasMore: false,
    loading: false,
    capacityReached: false,
    paginationError: false
  });
  const [transcriptHistoryState, setTranscriptHistoryState] = react.useState({
    hasMore: false,
    loading: false,
    capacityReached: false,
    paginationError: false
  ...
  const store = react.useMemo(
    () => daemon.createDaemonTranscriptStore({
      maxBlocks,
  ...
  const eventStreamRef = react.useRef(void 0);
  const lastSessionIdRef = react.useRef(void 0);
  const activePromptsRef = react.useRef(/* @__PURE__ */ new Map());
  const settledPromptsRef = react.useRef(/* @__PURE__ */ new Map());
  const pendingSessionLoadRef = react.useRef(
    void 0
  );
  const pendingSessionLoadIdRef = react.useRef(0);
  const liveJournalRepairRef = react.useRef(
    void 0
  );
  const repairReloadRef = react.useRef(void 0);
  const tryLiveJournalRepairRef = react.useRef(void 0);
  const passiveAssistantDoneTimerRef = react.useRef(void 0);
  const heartbeatSupportedRef = react.useRef(false);
  const heartbeatFailureStateRef = react.useRef({
    consecutiveFailures: 0
  });
  const manualSessionClearRef = react.useRef(false);
  const skipNextCleanupDetachSessionRef = react.useRef(void 0);
  const settledRestoredActivePromptSessionsRef = react.useRef(/* @__PURE__ */ new WeakSet());
  const eventOptionsRef = react.useRef({ suppressOwnUserEcho, includeRawEvent });
  const reconnectConfigRef = react.useRef({ reconnectDelayMs, maxReconnectDelayMs });
  const reconnectAbortRef = react.useRef(void 0);
  const loadWarningsRef = react.useRef(loadWarnings);
  const historyPageSizeRef = react.useRef(historyPageSize);
  const subagentTranscriptModeRef = react.useRef(subagentTranscriptMode);
  const clientIdRef = react.useRef(getStableClientId(clientId));
  ...
  const createSessionRequestRef = react.useRef(createSessionRequest);
  createSessionRequestRef.current = createSessionRequest;
  const [promptStatus, setPromptStatus] = react.useState("idle");
  const [restoreSessionId, setRestoreSessionId] = react.useState(
    initialRestoreSessionId
  );
  const [restoreWorkspaceCwd, setRestoreWorkspaceCwd] = react.useState(void 0);
  const [restoreMode, setRestoreMode] = react.useState("load");
  const [restoreSessionNonce, setRestoreSessionNonce] = react.useState(0);
  const [attachSessionNonce, setAttachSessionNonce] = react.useState(0);
  const [newSessionNonce, setNewSessionNonce] = react.useState(0);
  ...
  const initialClientIdDependencyRef = react.useRef(clientId);
  ...
  const setConnectionSynchronous = react.useCallback(
    (update) => {
  ...
  const addNotice = react.useCallback((input) => {
    const notice = {
      ...input,
  ...
  const clearNotices = react.useCallback(() => {
    setNotices([]);
  }, []);
  const noticesValue = react.useMemo(
    () => ({
      notices,
      dismissNotice,
      clearNotices
  ...
  const [workspaceEventSignals, setWorkspaceEventSignals] = react.useState(INITIAL_WORKSPACE_EVENT_SIGNALS);
  const hasCurrentSessionActivePromptRef = react.useRef(() => false);
  ...
  react.useEffect(() => {
    ...
    const run = async () => {
      ...
      const client = workspaceClientRef.current ?? new daemon.DaemonClient({ baseUrl: resolvedBaseUrl, token: resolvedToken });
      let session;
      let capabilities;
      ...
      while (!disposed && !abort.signal.aborted) {
        ...
        try {
          ...
          if (!session) {
            ...
            const restoreMethod = restoreSessionId && restoreMode === "resume" && !legacyClientRebind ? daemon.DaemonSessionClient.resume : daemon.DaemonSessionClient.load;
            ...
            const nextSession = restoreSessionId ? await restoreMethod(
              ...
              restoreSessionId,
              {
                ...
                ...restoreMethod === daemon.DaemonSessionClient.load && subagentTranscriptModeRef.current === "summary" ? { liveReplayMode: "summary" } : {},
              ...
              requestClientId
            ) : reconnectSessionId ? await daemon.DaemonSessionClient.load(
              client,
              reconnectSessionId,
              {
                workspaceCwd: effectWorkspaceCwd,
              ...
              requestClientId
            ) : await daemon.DaemonSessionClient.createOrAttach(
              client,
              {
          ...
          if (replayInjected) {
            ...
            if (rebuildReplay) {
              ...
              const replayStore = daemon.createDaemonTranscriptStore(
          ...
          if (loadWarningTexts.length > 0) {
            ...
            if (warningEvents.length > 0 && (repair == null ? void 0 : repair.sessionId) === activeSession.sessionId) {
              const checkpointStore = daemon.createDaemonTranscriptStore({
                ...repair.checkpoint,
          ...
          for await (const event of activeSession.events({
            ...
            try {
              ...
              const hasBlockPathDebugEvent = uiEvents.some(
                (e) => e.type === "debug" && !daemon.isUnrecognizedDiagnosticReason(e.debugReason)
              );
  ...
  const actions = react.useMemo(
    () => createDaemonSessionActions({
      ...
      createDetachedSession: (workspaceCwd2, overrides) => {
        var _a;
        const client = workspaceClientRef.current ?? new daemon.DaemonClient({
          baseUrl: resolvedBaseUrl,
          token: resolvedToken
        });
        const request = {
        ...
        return daemon.DaemonSessionClient.createOrAttach(
          client,
          request,
          requestClientId
        );
  ...
  const loadMoreTranscript = react.useCallback(
    async (options) => {
      var _a;
      const history = transcriptHistoryRef.current;
      const activeSession = sessionRef.current;
      ...
      try {
        ...
        const retryable = !terminalFailure && (!(error instanceof daemon.DaemonHttpError) || error.status >= 500 || error.status === 408 || error.status === 429);
  ...
  const transcriptHistoryValue = react.useMemo(() => {
    var _a;
  ...
  const lastHandledSessionIdRef = react.useRef(UNHANDLED_SESSION);
  const lastHandledWorkspaceRef = react.useRef(void 0);
  const lastHandledClientIdRef = react.useRef(void 0);
  react.useEffect(() => {
  ...
  const ownerGuardValue = react.useMemo(
    () => ({
      capture: () => {
        const session = sessionRef.current;
  ...
  return /* @__PURE__ */ jsxRuntime.jsx(DaemonStoreContext.Provider, { value: store, children: /* @__PURE__ */ jsxRuntime.jsx(DaemonConnectionContext.Provider, { value: connection, children: /* @__PURE__ */ jsxRuntime.jsx(DaemonPromptStatusContext.Provider, { value: promptStatus, children: /* @__PURE__ */ jsxRuntime.jsx(DaemonSessionNoticesContext.Provider, { value: noticesValue, children: /* @__PURE__ */ jsxRuntime.jsx(
    DaemonWorkspaceEventSignalsContext.Provider,
    {
      value: workspaceEventSignals,
      children: /* @__PURE__ */ jsxRuntime.jsx(DaemonActionsContext.Provider, { value: actions, children: /* @__PURE__ */ jsxRuntime.jsx(
...
function settleActivePromptFromTurnEvent(activePrompts, settledPrompts, sessionId, event, store, setPromptStatus, passiveAssistantDoneTimerRef, opts = {}) {
  ...
  clearPassiveAssistantDoneTimer(passiveAssistantDoneTimerRef);
  try {
    const result = daemon.matchTurnEvent(event, promptId);
...
function normalizeAndFilterEvent(event, clientId, opts, setConnection, behavior = {}) {
  ...
  const normalized = daemon.normalizeDaemonEvent(event, {
    clientId,
...
function useDaemonTranscriptStore() {
  const store = react.useContext(DaemonStoreContext);
  if (!store) {
    throw new Error(
...
function useDaemonTranscriptHistory() {
  const history = react.useContext(DaemonTranscriptHistoryContext);
  if (!history) {
    throw new Error(
...
function useDaemonTranscriptBlocks() {
  const store = useDaemonTranscriptStore();
  const getBlocks = react.useCallback(() => store.getSnapshot().blocks, [store]);
  return react.useSyncExternalStore(store.subscribe, getBlocks, getBlocks);
}
function useDaemonPendingPermissions() {
  const blocks = useDaemonTranscriptBlocks();
  return react.useMemo(() => selectDaemonPendingPermissions(blocks), [blocks]);
}
function useDaemonActiveTodoList() {
  const blocks = useDaemonTranscriptBlocks();
  return react.useMemo(() => selectDaemonActiveTodoList(blocks), [blocks]);
}
function useDaemonStreamingState() {
  const store = useDaemonTranscriptStore();
  const promptStatus = useDaemonPromptStatus();
  const getStreamingState = react.useCallback(
...
function useDaemonActions() {
  const actions = react.useContext(DaemonActionsContext);
  if (!actions) {
    throw new Error(
...
function useOptionalDaemonActions() {
  return react.useContext(DaemonActionsContext);
}
function useDaemonSessionOwnerGuard() {
  const guard = react.useContext(DaemonSessionOwnerGuardContext);
  if (!guard) {
    throw new Error(
...
function useDaemonWorkspaceEventSignals() {
  return react.useContext(DaemonWorkspaceEventSignalsContext);
}
function useDaemonPromptStatus() {
  const promptStatus = react.useContext(DaemonPromptStatusContext);
  if (!promptStatus) {
    throw new Error(
...
function useDaemonConnection() {
  const connection = react.useContext(DaemonConnectionContext);
  if (!connection) {
    throw new Error(
...
function useDaemonSessionNotices() {
  const value = react.useContext(DaemonSessionNoticesContext);
  if (!value) {
    throw new Error(
...
function isClosingSessionLoadError(error, allowLegacyMessage = false) {
  if (!(error instanceof daemon.DaemonHttpError) || error.status !== 404) return false;
...
function isRestoreInProgressLoadError(error) {
  if (!(error instanceof daemon.DaemonHttpError) || error.status !== 409) return false;
  const body = isRecord(error.body) ? error.body : void 0;
...
function useDaemonChannels(options = {}) {
  ...
  const previousWorkspaceRef = react.useRef(workspaceCwd);
  const reload = react.useCallback(async () => {
  ...
  const setStartup = react.useCallback(
    (name, request) => mutate(
...
function useDaemonDiagnostics(options = {}) {
  ...
  const loadPreflight = react.useCallback(
...
function useDaemonFollowupSuggestion(opts = {}) {
  ...
  const lastFollowupSuggestion2 = react.useSyncExternalStore(
    store.subscribe,
    () => store.getSnapshot().lastFollowupSuggestion,
    () => store.getSnapshot().lastFollowupSuggestion
  );
  const sidechannelFollowupSuggestion = react.useSyncExternalStore(
  ...
  const lastPushedPromptIdRef = react.useRef(void 0);
  ...
  const onAcceptFollowup = react.useCallback(
    (method, options) => {
  ...
  const onDismissFollowup = react.useCallback(() => {
    controller.dismiss();
    clearStoreFollowupSuggestion(store);
    clearSidechannelFollowupSuggestion();
    lastPushedPromptIdRef.current = void 0;
...
function useDaemonMidTurnInjected() {
  const batches = react.useSyncExternalStore(
...
exports.AGENT_TOOL_NAME = toolNames.AGENT_TOOL_NAME;
exports.isAgentTool = toolNames.isAgentTool;
Object.defineProperty(exports, "DAEMON_APPROVAL_MODES", {
  enumerable: true,
  get: () => daemon.DAEMON_APPROVAL_MODES
});
exports.DAEMON_SESSION_DEFAULT_MAX_BLOCKS = DEFAULT_MAX_BLOCKS;
exports.DaemonSessionProvider = DaemonSessionProvider;
exports.DaemonWorkspaceProvider = DaemonWorkspaceProvider;
exports.consumePendingPromptEvents = consumePendingPromptEvents;
exports.getPendingPromptEvents = getPendingPromptEvents;
exports.getPendingPromptVersion = getPendingPromptVersion;
exports.isMissingSessionHttpStatus = isMissingSessionHttpStatus;
exports.subscribePendingPromptEvents = subscribePendingPromptEvents;
exports.subscribePendingPromptVersion = subscribePendingPromptVersion;
exports.useActions = useDaemonActions;
exports.useActiveTodoList = useDaemonActiveTodoList;
exports.useAgents = useDaemonAgents;
exports.useAuth = useDaemonAuth;
exports.useChannels = useDaemonChannels;
exports.useConnection = useDaemonConnection;
exports.useDaemonFollowupSuggestion = useDaemonFollowupSuggestion;
exports.useDaemonMidTurnInjected = useDaemonMidTurnInjected;
exports.useDaemonSessionOwnerGuard = useDaemonSessionOwnerGuard;
exports.useDiagnostics = useDaemonDiagnostics;
exports.useFiles = useDaemonFiles;
exports.useGlob = useDaemonGlob;
exports.useMcp = useDaemonMcp;
exports.useMemory = useDaemonMemory;
exports.useOptionalWorkspace = useOptionalDaemonWorkspace;
exports.usePendingPermissions = useDaemonPendingPermissions;
exports.usePromptStatus = useDaemonPromptStatus;
exports.useProviders = useDaemonProviders;
exports.useResource = useDaemonResource;
exports.useSession = useDaemonSession;
exports.useSessionNotices = useDaemonSessionNotices;
exports.useSessions = useDaemonSessions;
exports.useSettings = useDaemonSettings;
exports.useSkills = useDaemonSkills;
exports.useStatusReport = useDaemonStatusReport;
exports.useStreamingState = useDaemonStreamingState;
exports.useTools = useDaemonTools;
exports.useTranscriptBlocks = useDaemonTranscriptBlocks;
exports.useTranscriptHistory = useDaemonTranscriptHistory;
exports.useTranscriptState = useDaemonTranscriptState;
exports.useTranscriptStore = useDaemonTranscriptStore;
exports.useUsageDashboard = useDaemonUsageDashboard;
exports.useWorkspace = useDaemonWorkspace;
exports.useWorkspaceActions = useDaemonWorkspaceActions;
exports.useWorkspaceEventSignals = useDaemonWorkspaceEventSignals;
//# sourceMappingURL=daemon-react-sdk.cjs.map
